...
public class DisplayGUI extends AnimationTimer {
    ...
    private synchronized void trackListener() {
        Train train;
        Track track;
        for (int i = 0; i < WIDTH; i++) {
            for (int j = 0; j < HEIGHT; j++) {
                if (map[i][j] != null && (map[i][j].
                        getClass().getSimpleName().equals("Track")
                ...
                if (map[i][j] != null && map[i][j].
                        getClass().getSimpleName().equals("Station")) {
                ...
                if (map[i][j] != null && map[i][j].
                    ...
                    if (((Track) map[i][j]).isSwitchD){
                        ...
                        if(track.direction == 0) {
                            ...
                            switchLine.setStartX(
                                    ((i * X_LENGTH) + X_LENGTH + (i * DX)));
                            switchLine.setStartY(
                                    ((j + 1) * Y_LENGTH) + Y_LENGTH);
                            switchLine.setEndX(
                                    (i * X_LENGTH) + (i - 1) * DX);
            ...
            for (int j = 0; j < HEIGHT; j++) {
                if (map[i][j] != null && map[i][j].
                    ...
                    if(temp.isSwitchD || temp.isSwitchU) {
                        ...
                        light.setCenterY(
                                ((line.getStartY() +
                                        line.getEndY()) / 2) - lightHeightOffset);
